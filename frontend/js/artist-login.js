 ...
 * artist-login.js
 *
 * Drives the artist-login.html page.
 *
 * Flow:
 *   1. On submit, validate that both fields are non-empty.
 *   2. POST /api/auth/artist/login with { email, password }.
 *   3. On success, redirect to artist-dashboard.html.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      await api.post('/auth/artist/login', { email, password });
      // Success — session cookie is set; redirect to artist dashboard.
